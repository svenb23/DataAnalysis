...
Word2Vec (100-dimensionale Vektoren) eingesetzt. Um beide Repräsentationen als Input für die
Themenextraktion zu vergleichen, wurde Word2Vec zusätzlich über K-Means-Clustering ausgewertet.

Die Themenextraktion (\texttt{04\_topic\_modeling.py}) erfolgte mit drei Ansätzen: LDA und NMF
auf der TF-IDF-Matrix sowie K-Means-Clustering auf den Word2Vec-Dokumentvektoren (jeweils 10~Topics).
Die Qualität wurde über den Coherence Score ($c_v$) quantifiziert (\cref{fig:coherence}): NMF erzielte
mit 0,65 einen deutlich höheren Wert als LDA mit 0,57 und lieferte schärfer abgegrenzte Topics
(siehe \cref{fig:topwords} im Anhang). K-Means auf Word2Vec produzierte generischere Topics
mit starken Überlappungen (\cref{fig:w2vclouds}), was TF-IDF als überlegenen Input für Topic Modeling bestätigt.
Die extrahierten Themen (z.\,B. Debt Collection, Identity Theft, Mortgage) stimmen mit den
Produktkategorien der Metadaten überein (siehe \cref{fig:products} im Anhang), was die
Validität der Ergebnisse stützt.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../results/w2v_kmeans_wordclouds.png}
\caption{K-Means (Word2Vec) -- Wortwolken pro Cluster}
\label{fig:w2vclouds}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{../results/coherence_comparison.png}
\caption{Vergleich der Coherence Scores ($c_v$): LDA vs.\ NMF}
\label{fig:coherence}
\end{figure}
